...
class VisualList {
    ...
    async eventMode(param) {
        ...
        ViewElement.addEventListener("scroll", async () => {
            ...
            let secondElementContainer = document.querySelector(secondElement);
            ...
            if (!isUpDirction && scrollTop > beforeHeight) {
                for (let i = 0; i < secondElementContainer?.children.length; i++) {
                    ...
                    if (scrollTop >= beforeHeight) {
                        ...
                        beforeScrollArr.push(secondElementContainer.children[i]?.offsetHeight);
                        this.startIndex++;
                        param.render(param.data.slice(this.startIndex, this.endIndex));
                        while (ViewElement.offsetHeight + offset >=
                ...
                for (let i = 0; i < beforeScrollArr.length; i++) {
                    ...
                    if (base > diff) {
                        ...
                        for (let j = i; j < beforeScrollArr.length; j++) {
                            if (document.querySelector(param.ParentElement).offsetHeight >
                                tempHeight) {
                        ...
                        this.render(param, this.startIndex, this.startIndex + 2, beforeHeight);
                        break;
                    }
                    else {
                        continue;
    ...
    async render(param, index, endindex, beforeHeight) {
