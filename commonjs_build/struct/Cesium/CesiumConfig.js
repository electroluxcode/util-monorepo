...
async function InitGltfModel(viewer, url = 'http://smartdeliver.oss-cn-beijing.aliyuncs.com/yt300mb.gltf', degrees, that) {
    ...
    if (!degrees) {
        degrees = BaseConfig_js_1.defaultData;
    }
...
class cesiumInit {
    ...
    constructor(idName) {
        ...
        this.container = new Cesium.Viewer(idName, BaseConfig_js_1.baseConfig);
        this.destroyedArr = [];
        this.initScene();
        this.initLight();
    }
    ...
    async initObject(type) {
        this.destroyedAll();
        if (type == 'bim') {
            this.ModelData = await InitGltfModel(this.container, 'http://smartdeliver.oss-cn-beijing.aliyuncs.com/yt300mb.gltf', BaseConfig_js_1.defaultData, this);
     ...
     */
    cameraMove(Mode = 'relative', DegreesType = BaseConfig_js_1.defaultData, config = { heading_toRadians: 0, pitch_toRadians: -45 }) {
        let destination;
        if (Mode == 'absolute') {
