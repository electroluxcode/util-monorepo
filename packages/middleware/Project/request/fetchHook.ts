...
import { middlewareFetch } from "./axiosMiddleWare.js";
import { fetchRetry } from "./fetchRetry.js";
import { fetchStatus } from "./fetchStatus.js";

class VAxios {
	private axiosInstance: easyFetch;
	...
	setupInterceptors() {
		...
		responseInterceptors = (axiosInstance: axiosInstanceType, res) => {
			let result;
		...
		responseInterceptorsCatch = (axiosInstance, error, res) => {
			...
			if (
				isFunction(
					axiosInstance?.config?.requestOptions?.retryRequest?.successFn
				)
			) {
				isSuccessFn = true;
			}
			if (
				isFunction(axiosInstance?.config?.requestOptions?.retryRequest?.errorFn)
			) {
				isErrorFn = true;
			}
			if (axiosInstance?.config?.requestOptions?.retryRequest?.isOpenRetry) {
