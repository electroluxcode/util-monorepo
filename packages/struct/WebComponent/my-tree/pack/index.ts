...
class tree {
	...
	constructor(options: OptionType) {
		...
		if (!this.options.multiple) {
			for (let i in this.TreePrefData) {
				...
				let dom = this.$container.querySelector(
					`[tree-id="${this.TreePrefData[i].key}"]`
				)! as HTMLElement;
	...
	CreateTreeNode(node: DataType, layer: number, UpdateTitle?: boolean) {
		...
		$title.setAttribute(
			"class",
			`node-title ${TitleSelectedClass} ${TitleSearchedClass}`
		);
			...
			for (let index = data.children.length - 1; index >= 0; index--) {
				...
				this.delTreeNode(
					...
					this.$container.querySelector(
						`[tree-id='${data.children[index].key}']`
					)! as HTMLElement
				);
			}
		}
	...
	searchParentSwitch(ParentId: string, StopFlag: string) {
		...
		if (this.TreePrefData[ParentId].parent != StopFlag) {
			...
			parent = parent.concat(
				this.searchParentSwitch(this.TreePrefData[ParentId].parent, "0")
			);
		} else {
			parent.push(this.TreePrefData[ParentId]);
		}
	...
	searchChildSwitch(childId: string) {
		...
		if (this.TreePrefData[childId].children) {
			...
			for (let i in this.TreePrefData[childId].children) {
				// @ts-ignore
				child = child.concat(
					this.searchChildSwitch(this.TreePrefData[childId]["children"]![i].key)
				);
			}
		} else {
			child.push(this.TreePrefData[childId]);
		...
		res.forEach((v: any) => {
			...
			for (let i = parentArr.length - 1; i >= 1; i--) {
				...
				setTimeout(
					(param) => {
						if (
							this.TreePrefData[param.key].expand ||
