...
type defineStoreType<t extends { state: any }> = {
	...
	actions: Record<any, (param: any) => any>;
	getState?: (prop: keyof t["state"]) => void;
	enhancer?: Record<any, (...prop: any) => void>;
	subscribe?: Record<any, (...prop: any) => void>;
};

export function defineStore<T extends defineStoreType<T>>(
	prop: T
): () => {
	...
	[key in keyof T["getters"]]: T["getters"][key];
} & {
	subscribe: any;
} {
	...
	return () => {
			...
			apply: function (target, thisArg, argumentsList) {
				...
				queueMicrotask(() => {
					listeners.forEach((listener) =>
						listener({ ...prop.state }, { ...prop.state })
					);
				});
		...
		return { ...proxy, ...prop.state, ...prop.getters, subscribe } as any;
	};
}
