...
class VisualList {
    ...
    constructor(param) {
        ...
        this.ViewObject.ViewElement.addEventListener("scroll", () => {
            // 尽量包括多一点东西
            let BeforeElementCount = Math.floor(this.ViewObject.ViewElement.scrollTop /
                this.ViewObject.SingleElementHeight);
            console.log("????", BeforeElementCount, BeforeElementCount + this.ViewObject.ViewElementCount + 1);
            this.TrueRenderData.init = {
                data: param.data.slice(BeforeElementCount, BeforeElementCount + this.ViewObject.ViewElementCount + 1),
                status: "update",
    ...
    ProxyGet(TrueRenderData) {
        ...
        ProxyRender.FnAdd(function () {
            ...
            if (this.init.status == "update") {
                that.ReRender(document.querySelector(".container_layer"), this.init.data, this.init.start, false);
            }
        });
        return ProxyRenderData;
    }
    ...
    ReRender(ui, data, start, first) {
        ...
        for (let i = 0; i < length; i++) {
            // console.log(data["data"][i])
            let temp = data[i].id;
            ui.children[i].style.top = `${(i + start) * this.ViewObject.SingleElementHeight}px`;
    ...
    FirstRender(data) {
        ...
        this.ViewObject.ViewElementCount = Math.ceil(this.ViewObject.ViewElementHeight / this.ViewObject.SingleElementHeight);
        let AllRenderElementHeight = this.TrueRenderData?.init.data.length *
            this.ViewObject.SingleElementHeight;
        ...
        this.ReRender(document.querySelector(".container_layer"), this.TrueRenderData?.init.data.slice(0, ViewElementCount), 0, true);
        let temp = [];
