	...
	eventMode(param: VisualListType) {
		...
		queueMicrotask(() => {
			...
			let shouldDisplayCount = Math.floor(
				param.ViewElement.clientHeight / ChildrenElement.clientHeight
			);
			...
			param.ViewElement.addEventListener("scroll", () => {
				...
				let BeforeElementCount = Math.floor(
					param.ViewElement.scrollTop / ChildrenElement.clientHeight
				);
				...
				param.render(
					param.data.slice(
						BeforeElementCount,
						BeforeElementCount + shouldDisplayCount
					)
				);
			});
		});
