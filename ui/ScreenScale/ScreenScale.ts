  ...
  public FixMap(el: string[] = ["#app"], level: string = "1",): void {
    ...
    for (let i in this.CurrelFixMap) {
      ...
      if (!this.IsMapElement) {
        ...
        (item as any).originBounding=item.getBoundingClientRect()
      ...
      let radio3 = (item as any).originalWidth / (item as any).originalHeight
      // 得到变化尺寸 用于自定义
      ...
      let radioHeight = (item as any).originalHeight/this.options.dh!;
      ...
      console.log("zptest:",{
        ...
        "目前大容器":this.dom,
        "radioHeight":[(item as any).originalHeight,this.options.dh,radioHeight],
      ...
      // (item as any).style.height = `${this.currScale *(item as any).originalHeight}px  `;
      (item as any).style.width = `${this.currScale *(item as any).originalWidth}px  `;
// debugger
